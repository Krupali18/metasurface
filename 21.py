...
def FoM(FamilyOfGrating):
    global BestFoMSoFar
    my_grating =Grating(lateral_period=lateral_period*nm, grating_period=grating_period*nm, cyl_height=220.0*nm, n_glass=1.45, n_tio2=4.2138, xyrra_list_in_nm_deg=np.array(FamilyOfGrating), data=None)    
    my_grating.get_xyrra_list(units='nm,deg', replicas=4)
    ...
    FigureOfMerit = my_grating.run_lua(subfolder='temp',target_wavelength=target_wavelength*nm,numG=50)
...
class GA:
    ...
    def __init__(self, StartingGuess=None, xmin=(-349.9999999), ymin=(-349.99999999),
                 xmax=grating_period+349.9999999, ymax=lateral_period+349.99999999,
                 majorAxisMin=50, MajorAxisMax=125,
                 minorAxisMin=50, MinorAxisMax=125,angleMin=(0.00001),
                 angleMax=(359.99999999),
                 FamilySize=5, IndividualsInSingleGen =1500, NumberOfGenerations=100, converged=False):
    ...
    def MutateIndividualsInFamily(self, Family, mutationRate = 0.03, maxTrials=2000):
            ...
            for eachMember in Family:
                ...
                if ScoreToMutate < mutationRate:
                        ...
                        while(not Done and counter<=maxTrials//50):
                            ...
                            counter += 1
                            for i in range(0,5):
                                Percentage_Change= random.uniform(0.00001,0.1200)
    ...
    def CrossingOfIndividualsOfDifferentFamilies(self, Individual_1, Individual_2, CrossingOverRate=0.95):
